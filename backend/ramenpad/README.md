...
- Durable progress in `ramenpad.indexer_state`, with two-block confirmation and idempotent log keys. After downtime, the saved cursor resumes from the exact next block and backfills missed launches, swaps, and fee events.
...
The health response also exposes `launchReady`. It remains false during startup/backfill and after indexing failures. The frontend checks it immediately before requesting a launch transaction, so routine deploys and restarts fail closed instead of allowing new launches while ingestion is unavailable.
